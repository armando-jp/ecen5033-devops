...
sed -i 's/V1/V2/g' ./pyapp/server.py

# modify the nginx-conf.d/server.conf file
sed -i 's/BLUE/GREEN/g' ./nginx-conf.d/server.conf
...
# BLUE environment is no longer needed so we remove it
docker stop python_server-BLUE
docker rm python_server-BLUE